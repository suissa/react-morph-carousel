...
type HoverEffectKey = keyof typeof hoverEffects;
...
type DirectionalTransitionConfig = {
...
type AnimationSettings = {
  autoplay: boolean;
  delay: number;
  hover: HoverEffectKey;
  selected: {
    animation: string;
    isSticky: boolean;
    onClick: (item: CarouselItem) => void;
  };
  transition: TransitionConfig | DirectionalTransitionConfig;
};
...
const animations: Record<string, AnimationSettings> = {
  fade: {
    autoplay: true,
    delay: 3000,
    hover: 'scaleUp',
  ...
  slideDynamic: {
    ...
    transition: {
      next: {
        out: 'opacity-0 -translate-x-full',
...
const CarouselCard: React.FC<{
...
}> = ({ item, isSelected, isHovered, animationConfig }) => {
  ...
  return (
    <div
      className={`w-full h-64 ${item.bgColor} rounded-lg shadow-lg flex flex-col justify-center items-center p-6 text-white text-center transition-all duration-300 ease-in-out ${hoverClasses} ${selectedClasses}`}
    >
...
export default function App() {
  const [items] = useState<CarouselItem[]>(sampleItems);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [animation, setAnimation] = useState<keyof typeof animations>('whirlpool');
  const [animationPhase, setAnimationPhase] = useState<'in' | 'out' | 'initial'>('in');
  ...
  const isStickyActive =
    selectedIndex !== null && animationConfig.selected.isSticky && itemsPerPage > 1;

  const handleNavigation = useCallback(
    (directionOrIndex: 'next' | 'prev' | number) => {
      if (isStickyActive) return;

      const newDirection: 'next' | 'prev' =
        typeof directionOrIndex === 'string'
          ? directionOrIndex
          : directionOrIndex > currentIndex
          ? 'next'
          : 'prev';
      ...
      window.setTimeout(() => {
        ...
        setCurrentIndex((prevIndex) => {
          if (typeof directionOrIndex === 'number') {
            return directionOrIndex;
          }

          const updatedIndex = newDirection === 'next' ? prevIndex + 1 : prevIndex - 1;

          if (updatedIndex >= totalPages) return 0;
          if (updatedIndex < 0) return totalPages - 1;
          return updatedIndex;
        });
    ...
    [currentIndex, isStickyActive, totalPages]
  );

  useEffect(() => {
    if (isPlaying && !isHovering && animationConfig.autoplay && !isStickyActive) {
      const timer = window.setInterval(() => handleNavigation('next'), animationConfig.delay);
      return () => window.clearInterval(timer);
    }
    return () => undefined;
  }, [animationConfig, handleNavigation, isHovering, isPlaying, isStickyActive]);
  ...
  const transition = animationConfig.transition;
  const currentTransitionConfig: TransitionConfig =
    'next' in transition ? transition[direction] : transition;
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col space-y-6">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-400">
            Selecione, passe o rato, e redimensione a janela.
          </p>
        </div>
        ...
        <div className="flex justify-center">
          <select
            value={animation}
            onChange={(event) => setAnimation(event.target.value as keyof typeof animations)}
